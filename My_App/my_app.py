...
class Main(flask.views.MethodView):
	...
	def post(self):
		...
		if username in users and users[username] == passwd:
			...
			flask.flash("User does not exist or incorrect password")
		return flask.redirect(flask.url_for('index'))
...
def login_required(method):
	...
	def wrapper(*args, **kwargs):
		if 'username' in flask.session:
			...
			flask.flash("A login in required to see the page!")
			return flask.redirect(flask.url_for('index'))
	return wrapper
...
class Music(flask.views.MethodView):
	@login_required
	def get(self):
		songs = os.listdir('static/music/') #seems to list a hidden file in addition to a song
